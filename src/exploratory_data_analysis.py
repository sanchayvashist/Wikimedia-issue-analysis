...
def parse_incident_report(plain_txt_dir):
    data_df = pd.DataFrame(columns=["summary", "detection", "conclusion", "actionable", "time"])
    ...
    for filename in tqdm(os.listdir(plain_txt_dir)):    
        ...
        summary = (summary_match.group(1) or summary_match.group(2)).strip() if summary_match else ""
    ...
    summary_count = data_df['Summary'].apply(lambda x: x is not None and str(x).strip() != "" and len(str(x)) >= 10).sum()
    print(f"Count of summaries not null, not empty, and at least 10 characters long: {summary_count}")
...
if __name__ == "__main__":
    parsed_df = parse_incident_report("src/data/plain_text")
    ...
    for batch_start in tqdm(range(0, len(summaries), batch_size), desc="Processing Batches"):
        batch_descriptions = summaries[batch_start:batch_start + batch_size]
        input_text = "SNo | summary\n" + "\n".join(
            [f"{batch_start + i+1} | {desc}" for i, desc in enumerate(batch_descriptions)]
        )
        
        json_data, usage_dict = categoies_summaries(prompt=prompt, input_text=input_text)
